...
public class Element {
    ...
    public float[] getDataElement(String name) {
    	Element element = getChildByName(name);
    	float[] data = new float[element.getChildCount()];
		for(int i = 0; i < data.length; i++) {
			data[i] = Float.parseFloat(element.getChild(i).text);
		}
